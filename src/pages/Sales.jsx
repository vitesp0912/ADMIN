    ...
    return (
      <div className="pf-page flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-ink-secondary font-medium">Loading sales...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="pf-page">
      <div className="mb-8">
        ...
        <p className="text-ink-secondary">View all sales transactions</p>
      </div>
      ...
      <div className="bg-surface rounded-xl shadow-lg border border-line p-6 mb-6 hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-5 rounded-xl border border-transparent hover:shadow-md transition-shadow">
            <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-2">Total Transactions</p>
            <p className="text-3xl font-bold text-ink">{filteredSales.length}</p>
          </div>
      ...
      <div className="bg-surface rounded-xl shadow-lg border border-line overflow-hidden hover:shadow-xl transition-shadow duration-300">
        {filteredSales.length === 0 ? (
          <div className="p-12 text-center bg-surface-muted rounded-xl border-2 border-dashed border-line-strong">
            <ShoppingCart className="w-16 h-16 text-ink-muted mx-auto mb-4" />
            <p className="text-ink-muted text-lg font-medium">No sales found</p>
          </div>
        ) : (
                ...
                {filteredSales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-green-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-ink">
                      ...
                      {sale.vehicle_number && (
                        <div className="text-xs text-ink-muted font-medium">{sale.vehicle_number.toUpperCase()}</div>
                      )}
                    </td>
                  </tr>
                ))}
