...
function Modal({ open, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 px-2">
      <div className="bg-surface rounded-xl shadow-lg p-4 sm:p-6 w-full max-w-md relative mx-auto" style={{maxWidth: 400}}>
        <button
          className="absolute top-2 right-2 text-ink-muted hover:text-ink-secondary text-xl font-bold"
          onClick={onClose}
          aria-label="Close"
        >
          ×
    ...
    return (
      <div className="pf-page flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-ink-secondary font-medium">Loading users...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="pf-page">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-ink-secondary">Manage all system users</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 border-2 border-line-strong rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all"
            />
          </div>
          <select
            value={filterRole}
            onChange={(e) => setFilterRole(e.target.value)}
            className="px-4 py-2 border-2 border-line-strong rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all font-medium"
          >
      ...
      <div className="bg-surface rounded-xl shadow-lg border border-line overflow-hidden hover:shadow-xl transition-shadow duration-300">
        {filteredUsers.length === 0 ? (
          <div className="p-12 text-center bg-surface-muted rounded-xl border-2 border-dashed border-line-strong">
            ...
            <p className="text-ink-muted text-lg font-medium">No users found</p>
          </div>
        ) : (
                ...
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-purple-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-semibold text-ink">{user.name || 'N/A'}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      >
                        ...
                        <Modal open={modalOpen} onClose={closePasswordModal}>
                          <h2 className="text-lg font-bold mb-2 text-ink">Set Password for {modalUser?.name || modalUser?.phone}</h2>
                          <div className="mb-4">
                            <label className="block text-sm font-medium text-ink-secondary mb-1">New Password</label>
                            <div className="relative flex items-center">
                              <input
                                type={showPassword ? 'text' : 'password'}
                                className="border border-line-strong rounded-lg px-3 py-2 w-full pr-10 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-base"
                          ...
                          <div className="flex gap-2 mt-4">
                            ...
                            <button
                              className="bg-gray-200 text-ink-secondary rounded px-4 py-2 font-semibold hover:bg-gray-300 transition-all duration-200"
      ...
      <div className="mt-6 bg-surface rounded-xl shadow-md border border-line p-4">
        <p className="text-sm font-medium text-ink-secondary">
          Showing <span className="font-bold text-ink">{filteredUsers.length}</span> of <span className="font-bold text-ink">{users.length}</span> users
        </p>
      </div>
    </div>
  )
