      ...
      <div className="bg-surface border-b border-line">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-ink">Pump Settings</h1>
              <p className="text-ink-secondary text-sm mt-1">Real-time operational metrics and fuel pricing</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-ink-muted">Loading pumps...</div>
          </div>
        ) : pumps.length === 0 ? (
          <div className="bg-surface rounded-lg shadow p-8 text-center">
            <Building2 className="w-12 h-12 text-gray-300 mx-auto mb-4" />
            <p className="text-ink-muted text-lg">No pumps found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            {/* Left Sidebar - Pump Selector */}
            <div className="lg:col-span-1">
              <div className="bg-surface rounded-lg shadow sticky top-8">
                <div className="p-4 border-b border-line bg-surface-muted">
                  <h2 className="text-sm font-semibold text-ink">SELECT PUMP</h2>
                </div>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 border-b border-line transition-colors ${
                        selectedPumpId === pump.id
                          ? 'bg-info-soft border-l-4 border-l-blue-600'
                          : 'hover:bg-surface-muted'
                      }`}
                    >
                      <div className="font-medium text-ink">{pump.name}</div>
                      <div className="text-xs text-ink-muted mt-1">{pump.pump_code}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-3">
              {metricsLoading ? (
                <div className="bg-surface rounded-lg shadow p-8 text-center">
                  <div className="text-ink-muted">Loading metrics...</div>
                </div>
              ) : !pumpMetrics ? (
                <div className="bg-surface rounded-lg shadow p-8 text-center">
                  <Fuel className="w-12 h-12 text-gray-300 mx-auto mb-4" />
                  <p className="text-ink-muted text-lg">Select a pump to view metrics</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Header */}
                  <div className="bg-surface rounded-lg shadow p-6 border-l-4 border-l-blue-600">
                    <h2 className="text-2xl font-bold text-ink">{pumpMetrics.pump_name}</h2>
                    <p className="text-ink-secondary mt-1">Code: {pumpMetrics.pump_code}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-surface rounded-lg shadow p-6">
                      <div className="flex items-end gap-3">
                        <div>
                          <p className="text-ink-secondary text-sm font-medium">NOZZLES</p>
                    ...
                    <div className="bg-surface rounded-lg shadow p-6">
                      <div className="flex items-end gap-3">
                        <div>
                          <p className="text-ink-secondary text-sm font-medium">FUEL TYPES</p>
                  ...
                  {pumpMetrics.fuel_types && Array.isArray(pumpMetrics.fuel_types) && pumpMetrics.fuel_types.length > 0 ? (
                    <div className="bg-surface rounded-lg shadow overflow-hidden">
                      <div className="px-6 py-4 bg-surface-muted border-b border-line">
                        <h3 className="text-sm font-semibold text-ink">FUEL PRICING</h3>
                      </div>
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-line">
                            <th className="px-6 py-3 text-left text-xs font-semibold text-ink-secondary">Fuel Type</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-ink-secondary">RSP</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-ink-secondary">RO</th>
                          </tr>
                        </thead>
                        <tbody>
                          {pumpMetrics.fuel_types.map((fuel, idx) => (
                            <tr key={idx} className="border-b border-line hover:bg-surface-muted">
                              <td className="px-6 py-3 text-sm text-ink font-medium">{fuel.name}</td>
                    ...
                    <div className="bg-surface rounded-lg shadow p-8 text-center">
                      <p className="text-ink-muted">No fuel types configured for this pump</p>
                    </div>
                  )}

                  {/* Data Integrity Note */}
                  <div className="bg-info-soft border border-transparent rounded-lg p-4">
